...
- **Composition timeline (2026-09-10, second pass — from your After Effects comparison)**: video clips now have a **position and a track** instead of only playing end-to-end. Drag a clip anywhere in time; drag it **up onto a new track** to stack it — the upper track plays on top where clips overlap, and the lower one resumes underneath afterwards (the same-track rule: a later clip cuts in over an earlier one). Empty timeline between clips renders as black + silence, in preview and in export. Internally the placed clips resolve to a flat edit-decision list (`lib/timeline.ts` → `computeEdl`), which is what the player walks and the exporter cuts; verified natively for overlays, gaps, same-track cuts and the keyframe-lead-in correction.
- **Multi-select + group operations**: click / Shift- or ⌘-click / drag a box on empty timeline / ⌘A. Move, Delete, ←/→ nudge, Split and **Repeat ×N** act on the whole selection as *one unit* — select everything and Repeat ×10 lays the entire arrangement down again ten times, tracks and lanes preserved. (Export always renders the whole timeline into one file; "Audio only" gives the combined result as an mp3 — it never extracted clips one by one.)
- **Smooth joins**: Project → *Smooth joins* sets an audio crossfade (0–2 s) applied at every cut between clips on export. It's built from the trimmed-away sound on each side of the cut ("handles"), so each piece keeps its exact length and **the video is never re-encoded**. Verified natively (`smooth.mts`): length preserved to the millisecond, continuous tone through the join, gaps untouched, mixed sample rates normalised. Not yet: a *video* crossfade/dip-to-black (that would re-encode the picture — available on request), and the live preview still hard-cuts (smoothing is applied in the export).
- **Convert to audio layer / repeat as audio (2026-09-10, third pass — "repeat it as audio, not the video")**: select any clips (one, a cut-and-joined pair, or ⌘A everything) and choose **To audio** on the timeline toolbar, *Convert to audio layer* in the right-click menu, or the Convert section in the inspector. The selection — its cuts, stacking, gaps and the Smooth-joins crossfade — is rendered **in the browser** (`lib/bounce.ts` → `renderClipsToAudio`, verified natively in `bounce.mts`) into one AAC file that lands on the timeline as an **audio layer at the same position**. Repeat ×N or Loop that layer and only the audio gets longer; the video stays exactly as it was. *Replace with audio layer* (menu) or the inspector's "remove the video clip" switch drops the video in the same step. Audio layers that run past the last clip now **extend the project** (`projectEnd`) — the preview keeps playing over black, and the export renders the tail as black video (or, with Export as audio / no clips left, just the audio). A project with only audio layers exports fine.
- **URL import codec fix (2026-09-11)**: an import that sat at "transferring to browser 100%" forever was yt-dlp choosing the *smallest* 1080p stream — AV1 video + Opus audio — which not every browser can decode; the thumbnail step then waited for a frame that never came. The server now sorts formats with `-S res:1080,vcodec:h264,acodec:m4a` (H.264 + AAC mp4, still capped at 1080p), which plays in every browser and is what the export stream-copies. Browser side: the thumbnail grab has a 10 s timeout, an unreadable file now fails with a clear message instead of a 0-second clip, and the progress line shows a "reading the file" phase after the transfer.
- **Timeline scrolling vs. follow-playhead (2026-09-17)**: on a long project (7 h) the timeline "kept jumping back" while playing — the follow-playhead auto-scroll re-centred the view every frame, so any manual scroll to reach another timestamp was undone instantly. Following now yields: a manual scroll suspends it until the playhead is back on screen (or playback restarts), and it never fires mid-scrub. The ruler now **scrubs on drag** (pointer-captured, like the playhead), not just on click, and no longer clears the selection when clicked. Ruler ticks are virtualized to the visible slice (`rulerTicks.ts`, tested) — zoomed in on a multi-hour project it used to try to render hundreds of thousands of tick elements. A seek issued while a long source is still loading its metadata now retargets the pending seek instead of being overridden by it.
- **Transport skip buttons (2026-09-17)**: the monitor's ⏮/⏭ buttons jumped to the previous/next *cut* — on a project with one short video followed by hours of audio, the only cut is at the video's end, so "forward" from anywhere landed at 3:14 every time (this was the "bounces back to 3 min" report). They are now **⏪ back 10 s / ⏩ forward 10 s** (Shift: 1 minute; keys J / L). Cut navigation moved to **↑ / ↓**, and **Home / End** go to the start / end of the project; stepping past the last cut now goes to the end rather than restarting that segment.
- **Range export (2026-09-17)**: Export used to render the whole timeline, so pulling ten minutes out of a seven-hour recording meant deleting everything else. Now mark an **In / Out range** on the timeline — `I` / `O` at the playhead, the *In* / *Out* / *= selection* buttons in the toolbar, right-click → *Export only this*, or drag the range handles on the ruler — and the Export dialog defaults to **Range** (with *Whole project* one click away). The EDL and the audio layers are sliced and re-based to the range (`sliceEdl` / `sliceLayers`, tested natively — a looped layer entered mid-cycle keeps its phase via a play-once head + loop), so cuts, gaps, stacking, smooth joins and layers all behave exactly as they did at that point of the full project. The file is named with the range (`name (01.02.00-01.12.00).mp3`). Clip trim-at-playhead moved from I/O to **[ / ]**.
- **Leading empty timeline is skipped on export (2026-09-17)**: trimming a clip's head in place leaves it starting later on the timeline (Start 56:50 for a clip trimmed at 56:50), and a whole-timeline export rendered that lead-in as silence — a 9-minute trim came out as a 66-minute file that was silent for 56 minutes. Export now skips empty timeline before the first clip/active layer by default (switch in the dialog to keep it); a marked range is explicit and always honoured as-is.

### Known limitations (updated)
- Same-track overlaps resolve as a cut to the later clip; there's no ripple/insert editing yet (clips don't push each other).
- Undo/redo still missing. Cut points on video are keyframe-snapped in the export (the measured-timeline correction keeps audio layers aligned); a re-encoding "frame-accurate cuts" mode would remove that at the cost of a full re-encode.
